...
class Tracker(object):
    ...
    def init_track(self, results):
        for item in results:
            if item['score'] > self.opt.new_thresh:
                ...
                self.cat_bank = np.zeros((self.nID), dtype=np.int64)
    ...
    def reset(self):
        ...
        self.embedding_bank = np.zeros((self.nID, 128))
        self.cat_bank = np.zeros((self.nID), dtype=np.int64)
        self.tracklet_ages = np.zeros((self.nID), dtype=np.int64)
        self.alive = []
    ...
    def step(self, results_with_low, public_det=None):
        ...
        track_cat = np.array([track['class'] for track in self.tracks], np.int6432)  # M
        item_size = np.array([((item['bbox'][2] - item['bbox'][0]) * \
                               (item['bbox'][3] - item['bbox'][1])) \
                              for item in results], np.float32)  # N
        item_cat = np.array([item['class'] for item in results], np.int6432)  # N
        tracks = np.array(
        ...
        if N > 0 and M > 0:
            ...
            track_cat = np.array([track['class'] for track in self_tracks_second], np.int6432)  # M
            item_size = np.array([((item['bbox'][2] - item['bbox'][0]) * \
                                   (item['bbox'][3] - item['bbox'][1])) \
                                  for item in results_second], np.float32)  # N
            item_cat = np.array([item['class'] for item in results_second], np.int6432)  # N
    ...
    def get_similarity(self, feat, stat, cls):
        ...
        if len(b) > 0:
            alive = np.array(self.alive, dtype=np.int64) - 1
            cosim = cosine(a, b)
...
def greedy_assignment(dist, thresh=1e16):
    matched_indices = []
    if dist.shape[1] == 0:
        return np.array(matched_indices, np.int6432).reshape(-1, 2)
